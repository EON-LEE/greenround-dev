...
class HighlightPoseAnalyzer(BasePoseAnalyzer):
    ...
    def analyze_for_highlight(self, video_path: str, progress_callback=None) -> Dict:
        ...
        try:
            ...
            logger.info(f"프레임 로딩 완료: {total_frames}개 프레임, {frame_load_time:.3f}s ({total_frames/frame_load_time:.1f} FPS)")
            
            # 프레임별 분석
            frames_data = []
            prev_pose_landmarks = None
            pose_analysis_start = time.time()
            ...
            pose_analysis_time = time.time() - pose_analysis_start
            
            total_fps = total_frames / pose_analysis_time if pose_analysis_time > 0 else 0
            logger.info(f"포즈 분석 완료: {total_frames}개 프레임, {pose_analysis_time:.3f}s ({total_fps:.1f} FPS)")
