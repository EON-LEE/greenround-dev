...
from ..common.utils import update_task_status, get_file_path, upload_to_gcs_and_get_public_url, TEMP_DIR
...
class HighlightEngine:
    ...
    def _generate_3stage_highlight(self, video_path: Path, swing_range: tuple, video_info: dict, output_path: Path, slow_factor: int, task_id: str):
        ...
        try:
            ...
            logger.info(f"비디오 프레임 로딩: {len(all_frames)}개 프레임, {frame_load_time:.3f}s")

            # 2. 3개의 클립 생성
            update_task_status(task_id, "processing", 40, "3단계 하이라이트 클립 생성")
            clip_gen_start = time.time()
            ...
            clip_gen_time = time.time() - clip_gen_start
            logger.info(f"클립 생성 완료: {clip_gen_time:.3f}s (클립1: {len(clip1)}, 클립2: {len(clip2)}, 클립3: {len(clip3)}프레임)")
            ...
            for i, current_clip in enumerate(clips_to_write):
                if i == 0:
                    frames_to_write = current_clip[:-transition_frames]
                    ...
                    frames_to_write = current_clip[transition_frames:-transition_frames]
                    for frame in frames_to_write: 
                        writer.write(frame)
                        total_frames_written += 1
            ...
            if clips_to_write:
                ...
                final_frames = last_clip[-transition_frames:]
                for frame in final_frames: 
                    writer.write(frame)
                    total_frames_written += 1
                    
            encode_time = time.time() - encode_start
            total_highlight_time = time.time() - highlight_start
            
            logger.info(f"하이라이트 영상 생성 완료: {total_frames_written}개 프레임, 인코딩 {encode_time:.3f}s, 전체 {total_highlight_time:.3f}s")
        finally:
            writer.release()
            cap.release()
